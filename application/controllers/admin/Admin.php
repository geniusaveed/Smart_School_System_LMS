   ...
   public function updatePurchaseCode()
    {
      ...
      $envato_market_purchase_code = $this->input->post('envato_market_purchase_code');
      $regex = "/[a-z0-9]-[a-z0-9]/";

            if (preg_match($regex, $envato_market_purchase_code)) {
                $this->load->library('encryption');
               $purchase_code= $this->encryption->encrypt($envato_market_purchase_code);
        ...
        $file_contents = str_replace('$config[\'SSLK\'] = \'' . $license . '\'', '$config[\'SSLK\'] = \'' .$purchase_code . '\'', $content);
        $update_handle = fopen($fname, 'w') or die("can't open file");
        if (fwrite($update_handle, $file_contents)) {
           $this->session->set_flashdata('message','Thank you for registering your product');
        ...
        fclose($update_handle);
                    ...
                    $this->session->set_flashdata('message','Purchase code not valid');
                     redirect('admin/admin/dashboard');
                }
        }
    }
